...
export const AccueilView: React.FC<AccueilViewProps> = ({
  ...
  const totalMonthRevenue = orders.reduce((sum, o) => sum + (o.paidFCFA || 0), 0);

  // Dynamic values or realistic defaults matching mockup
  const displayCaJour = '85 000 FCFA';
  const displayCaMois = totalMonthRevenue > 0 ? `${totalMonthRevenue.toLocaleString('fr-FR')} FCFA` : '1 240 000 FCFA';
  const displayActiveOrdersCount = activeOrders.length > 0 ? activeOrders.length : 18;
  const displayLateOrdersCount = lateOrders.length > 0 ? lateOrders.length : 2;
  const displayUnpaidBalance = totalUnpaidBalance > 0 ? `${totalUnpaidBalance.toLocaleString('fr-FR')} FCFA` : '350 000 FCFA';
  const displayUnpaidOrdersCount = unpaidOrdersCount > 0 ? unpaidOrdersCount : 12;

  // Mockup fallback list of upcoming deliveries matching screenshot exactly
  const mockUpcomingDeliveries = [
    {
      id: 'ord_24',
      orderNumber: 'Commande #024',
      clientName: 'Kossi A.',
      clientId: 'c1',
      dateText: 'Demain 10:00',
    ...
    {
      id: 'ord_28',
      orderNumber: 'Commande #028',
      clientName: 'Akouvi E.',
      clientId: 'c2',
      dateText: '15 Mai 15:00',
      badgeLabel: 'À livrer',
    ...
    {
      id: 'ord_31',
      orderNumber: 'Commande #031',
      clientName: 'Jean P.',
      clientId: 'c3',
      dateText: '16 Mai 11:00',
      badgeLabel: 'Prête',
    ...
    {
      id: 'ord_34',
      orderNumber: 'Commande #034',
      clientName: 'Komlan D.',
      clientId: 'c4',
      dateText: '19 Mai 09:00',
      badgeLabel: 'À venir',
      badgeColor: 'bg-[#F3F4F6] text-[#4B5563]',
      avatarBg: 'bg-[#1E112A] text-white',
      avatarInitial: 'K',
    },
  ];

  const actualUpcomingDeliveries = orders.slice(0, 4).map((o, idx) => {
    const mfg = o.manufacturingStatus || OrderEngine.mapLegacyToManufacturingStatus(o.status);
    const due = o.dueDateStatus || OrderEngine.calculateDueDateStatus(o.deliveryDate, mfg);
  ...
  const displayDeliveries = actualUpcomingDeliveries.length > 0 ? actualUpcomingDeliveries : mockUpcomingDeliveries;
      ...
      <div className="px-4 pt-1 pb-4 space-y-3 max-w-md mx-auto md:max-w-2xl lg:max-w-4xl">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="p-3.5 sm:p-4 rounded-[20px] bg-[#059669] text-white shadow-xs space-y-1.5 relative overflow-hidden">
            ...
            <div className="text-[11px] font-semibold text-white/90">↗ +12% vs hier</div>
          </div>
          ...
          <div className="p-3.5 sm:p-4 rounded-[20px] bg-[#6D28D9] text-white shadow-xs space-y-1.5 relative overflow-hidden">
            ...
            <div className="text-[11px] font-semibold text-white/90">↗ +18% vs mois dernier</div>
          </div>
          ...
          >
            ...
            <div className="text-[11px] font-semibold text-white/90 flex items-center gap-1">
              <span>⏱ {displayLateOrdersCount} en retard</span>
            </div>
          </div>
          ...
          >
            ...
            <div className="text-[11px] font-semibold text-white/90">Sur {displayUnpaidOrdersCount} commandes</div>
          </div>
        </div>
          ...
          <div className="flex items-center space-x-3 min-w-0">
            ...
            <div className="min-w-0">
              <h4 className="text-sm font-bold text-white truncate">
                {displayLateOrdersCount} commandes en retard de livraison
              </h4>
